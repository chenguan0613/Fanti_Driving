        ...
        if feature.face_detected:
            ...
            if window_buffer.is_ready():
                ...
                if perclos > 0.20:
                    ...
                    cv2.putText(frame, "WARNING: FATIGUE DETECTED!", (20, 150), 
                                cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 3)
                # 将数据渲染到视频画面 (HUD)
                cv2.putText(frame, f"PERCLOS: {perclos*100:.1f}%", (20, 70), cv2.FONT_HERSHEY_SIMPLEX, 0.7, hud_color, 2)
                cv2.putText(frame, f"EAR Mean: {ear_mean:.2f}", (20, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.7, hud_color, 2)
                cv2.putText(frame, f"MAR Max (Yawn): {mar_max:.2f}", (350, 70), cv2.FONT_HERSHEY_SIMPLEX, 0.7, hud_color, 2)
                cv2.putText(frame, f"Nodding (Pitch Std): {pitch_std:.1f}", (350, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.7, hud_color, 2)
                
            else:
                # 缓冲区还没满（刚启动的前几秒）
                loading_progress = int((window_buffer.current_size / window_buffer.window_size) * 100)
                cv2.putText(frame, f"Buffering... {loading_progress}%", (20, 70), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
        else:
            # 没检测到人脸，清空记忆防止数据污染
            cv2.putText(frame, "NO FACE DETECTED", (20, 70), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 3)
            window_buffer.clear()
        # 显示系统运行 FPS
        cv2.putText(frame, f"FPS: {fps:.1f}", (20, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2)
